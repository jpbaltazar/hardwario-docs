...
To use these sensors, the device must enroll them first. To enroll a tag, place it in very close proximity to CHESTER, run the following command and wait up to 10 seconds for the device to be discovered. A custom signal strength threshold (from -128 to 0dbm, -128dbm providing the most range, -40dbm being the default) can be added to make the enrollment less/more strict.
